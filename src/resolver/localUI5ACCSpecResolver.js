...
var DEFAULT_SUITES_GLOB = '**/test/**/acc/specs/acc.suite.js';
var DEFAULT_SUITES_EXCLUDE = 'node_modules';
var DEFAULT_SUITES_REGEX = '((?:\\w\\:)?\\/(?:[\\w\\-.]+\\/)+(?:[\\w\\.]+)\\/test\\/([\\w\\/]+)+acc\\/)specs\\/acc\\.suite\\.js';
          ...
          suitePaths.forEach(function (suitePath) {
            ...
            specFileNames.forEach(function (specFileName) {
              ...
              var spec = {
                ...
                lib: libName,
                branch: branch,
                testPath: suiteBasePath + '/specs/' + specFileName,

                // example: https://ui5.sap.com/test-resources/sap/m/acc/Button.html?sap-ui-language=en&sap-ui-theme=sap_fiori_3&sap-ui-rtl=false
                contentUrl: that.baseUrl ?
                  (that.baseUrl + '/' + that.contentRootUri + '/' + libName.replace(/\./g, '/').replace('/specs', '') + '/acc/' + specName + '.html') :
                  false
              };
